...
export default function TaxInputs({
  ...
  return (
    <div style={{ border: "1px solid #2a2e3a", borderRadius: 12, padding: "18px 18px 14px", background: "#181b23" }}>

      <div style={sLabel}>Your details</div>

      {/* Filing status + State: two-column */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
        <div>
          <label style={iLabel}>Filing status</label>
          <select value={status} onChange={(e) => setStatus(e.target.value)} style={selectStyle}>
            {Object.entries(FILING_LABELS).map(([v, l]) => <option key={v} value={v}>{l}</option>)}
          </select>
        </div>
        <div>
          <label style={iLabel}>State</label>
      ...
      <div style={{ height: 1, background: "#222530", margin: "4px 0 14px" }} />

      <div style={sLabel}>Income</div>
      ...
      <div style={{ height: 1, background: "#222530", margin: "8px 0 14px" }} />

      {/* W-2 toggle */}
      <div style={{ marginBottom: 12 }}>
        <label style={iLabel}>Do you also have W-2 income?</label>
      ...
      {/* W-2 inputs: two-column pairs */}
      {hasW2 && (
        <div style={{ marginBottom: 4 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <CurrencyInput label="W-2 annual income" value={w2Income} onChange={setW2Income} />
            <CurrencyInput label="Withholding this year" value={w2Withholding} onChange={setW2Withholding} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            <div style={{ marginBottom: 12 }}>
              <label style={iLabel}>Pay frequency</label>
            ...
            <div style={{ marginBottom: 12 }}>
              <label style={iLabel}>Paychecks left</label>
      ...
      <div style={{ height: 1, background: "#222530", margin: "4px 0 12px" }} />
